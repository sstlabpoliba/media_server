...
str = g_strdup_printf (
        "(" 
	" rtspsrc location=%s name=d d. ! queue2 ! rtph264depay ! avdec_h264 max-threads=6 ! videoconvert !  edgedetect ! videoconvert ! x264enc tune=zerolatency byte-stream=true bitrate=1024 speed-preset=superfast ! queue ! rtph264pay pt=96 name=pay0 "
	")", argv[3]);
